...
class MyDataset_L(Dataset):
    ...
    def __getitem__(self, index):
        ...
        w = 224
        i = 0
        wl = int(new_width / 2) - 224
        hl = int(new_height / 2) - 224
        while i < 3:
            # 随机产生x,y   此为像素内范围产生
            x = random.randint(1, wl)
            y = random.randint(1, hl)
            # 裁剪
            box = (x + new_width / 4, y + new_height / 4, w + x + new_width / 4, h + y + new_height / 4)
            imgi = img.crop(box)
