"""
提取局部特征,从每个图像中随机取3个224*224大小的块放入预训练的ResNet-50中
损失在第个周期左右收敛
得到局部训练的模型
"""
...
from dataset_L_random import MyDataset_L, DataLoader
...
writer = SummaryWriter('./log/' + TIMESTAMP, comment='randomline')
    ...
    for phase in ['train', 'val']:
        ...
        if phase == 'train':
            ...
            if (epoch + 1) % 5 == 0:
                ...
                torch.save(model, '/home/xf/model/resnet_epoch{}_l_8r.pkl'.format(epoch + 1))
        ...
        # deep copy the model
        if phase == 'val':
            writer.add_scalar('acc0/epoch', float(class0)/393, epoch)
            writer.add_scalar('acc1/epoch', float(class1) / 117, epoch)
            writer.add_scalar('acc2/epoch', float(class2) / 90, epoch)
...
# save best model
torch.save(model, "/home/xf/model/isic_resnet_l_8r.pkl")
